...
export function a2CompileAuthoringPlan(): AuthoringPlan {
  return buildAuthoringPlan({
    ...
    schemaVersion: '2.1.0',
    baseline: {
      baselineSnapshotId: 'baseline-sir-compiler',
      baselineSha256: hash,
    ...
    allowedTactics: [],
    adjacentCriteria: [
      { criterionHandle: 'criterion_001', criterionId: 'AP-A2', boundarySummary: 'Paired anti-pattern boundary for the suitability decision.' },
      { criterionHandle: 'criterion_002', criterionId: 'A1', boundarySummary: 'Purpose-boundary dependency that must remain distinct from suitability.' }
    ]
  });
}

export function completeSirCompileSnapshot(): PairCoherenceSnapshot {
  ...
  const evidence = materializeSirEvidence({
    capabilityEvidence: [
      ...
      },
      {
        ...
        claimSupported: 'Credible alternatives were considered before selecting AI.',
        evidenceClass: 'DECISION_RECORD',
        minimumTechnicalAssurance: 'IMPLEMENTED',
        requiredHumanAssurance: 'HUMAN_VALIDATED',
        acceptanceConditions: ['Analysis covers credible alternatives.'],
        limitations: ['A post-hoc rationale is insufficient.'],
        supportsAtomicHandles: ['atomic_002']
      },
      {
        title: 'Suitability validation',
        claimSupported: 'Current evidence supports the claimed suitability and expected value.',
        evidenceClass: 'TEST_RESULT',
        minimumTechnicalAssurance: 'TESTED',
        requiredHumanAssurance: 'HUMAN_VALIDATED',
        acceptanceConditions: ['Validation is current and scope-aligned.'],
        limitations: ['Out-of-scope testing is insufficient.'],
        supportsAtomicHandles: ['atomic_003']
      }
    ],
    antipatternEvidence: [
      {
      ...
      },
      {
        ...
        claimSupported: 'Decision chronology can show whether AI was predetermined.',
        evidenceClass: 'DECISION_RECORD',
        minimumTechnicalAssurance: 'IMPLEMENTED',
        requiredHumanAssurance: 'HUMAN_VALIDATED',
        acceptanceConditions: ['Chronology is attributable and complete enough.'],
        limitations: ['Timing alone does not prove motive.'],
        supportsAtomicHandles: ['atomic_002']
      },
      {
        title: 'Independent absence test',
        claimSupported: 'Independent testing can support presence, uncertainty or tested absence.',
        evidenceClass: 'INDEPENDENT_TEST',
        minimumTechnicalAssurance: 'TESTED',
        requiredHumanAssurance: 'FORMALLY_APPROVED',
        acceptanceConditions: ['Testing covers the defined mechanism and scope.'],
        limitations: ['Silence cannot establish tested absence.'],
        supportsAtomicHandles: ['atomic_003']
      }
    ],
  ...
  const sourceMappings: MaterializedSirSourceMappings = {
    ...
    capability: [
      {
        ...
        sourceId: 'SRC-EU-AIA',
        sourceVersionOrDate: '2024-06',
        exactLocator: 'Article 9(2)',
        relationship: 'SUPPORTS',
        supportedClaim: 'Suitability decisions remain bounded to a defined purpose and proportionate alternative analysis.',
        ...
        authorityType: 'LEGAL',
        locatorContextSha256: '3'.repeat(64)
      }
    ],
    antipattern: [],
    unmappedClaims: [
    ...
    mappingNotes: ['Capability has one exact locator; remaining anti-pattern claims stay explicitly unmapped.']
  };
  ...
  const controlBoundary: SirControlBoundaryOutput = {
    capabilityHardGate: {
      effect: 'CONSTRAIN',
      conditions: ['Material suitability evidence remains unresolved.'],
      overrideAuthority: 'Designated accountable human authority'
    },
    antipatternHardGate: {
      effect: 'BLOCK',
      conditions: ['Solution-first failure mechanism is confirmed.'],
      overrideAuthority: 'Designated accountable human authority'
    },
    capabilityRuntimeBoundary: {
      machineMay: ['Summarize validated evidence.'],
      machineMustNot: ['Approve progression.'],
      humanAuthorityRequiredFor: ['Any progression or exception decision.']
    },
    antipatternRuntimeBoundary: {
      machineMay: ['Surface validated failure indicators.'],
      machineMustNot: ['Accept residual risk.'],
      humanAuthorityRequiredFor: ['Any residual-risk or progression decision.']
    },
  ...
  return {
    ...
    apAbsence: {
      ...
      interpretationBoundary:
        'TESTED_ABSENT requires scoped, executed, successful, current and independently verified testing.'
    },
    sourceMappings,
    findings,
    controlBoundary,
