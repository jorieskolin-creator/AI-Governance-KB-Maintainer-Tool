...
const boundarySchema = z.object({
  capability: z.object({
    ...
    ownedTopics: strings,
    excludedTopics: z.array(z.object({
      criterionHandle: nonEmpty,
      ownershipBoundary: meaningful
    }).strict())
  }).strict(),
  antipattern: z.object({
    canonicalDefinition: meaningful,
    pairedRelationship: meaningful
  }).strict(),
  ...
  for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
    ...
    if (forbiddenIdentityKeys.has(key)) {
      findings.push(
        finding(
          context,
          'SIR_CANONICAL_IDENTITY_FORBIDDEN',
          childPath,
          `${key} is compiler/Authoring Plan owned and must not appear in model SIR output.`
        )
      );
    }
...
function validateBoundaryHandles(
  output: Record<string, unknown>,
  context: SirInitialCompletionContext,
  findings: ValidationFinding[]
): void {
  const allowed = new Set(context.authoringPlan.adjacentCriteria.map((item) => item.criterionHandle));
    ...
    if (!allowed.has(handle)) {
      findings.push(
        finding(
          ...
          `/capability/excludedTopics/${index}/criterionHandle`,
          `${handle} is not present in the sealed Authoring Plan adjacent-criteria universe.`
        )
      );
    }
  }
...
function validateQuestionSlots(
  output: Record<string, unknown>,
  context: SirInitialCompletionContext,
  findings: ValidationFinding[]
): void {
      ...
      if (item.slot !== expectedSlot) {
        findings.push(
          finding(
            ...
            `/${path}/${index}/slot`,
            `Expected governed slot ${expectedSlot}; canonical question ID and dimension will be materialized later.`
          )
        );
      }
    });
...
export function validateSirInitialCompletion(
  ...
  if (contract.taskType === 'PAIR_BOUNDARY') {
    validateBoundaryHandles(record, context, findings);
  }
  if (contract.taskType === 'PRIMARY_QUESTIONS') {
