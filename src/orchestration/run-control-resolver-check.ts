...
const base={pairRunId:'pair-run-control-resolver',authoringPlan:plan,categoryBaseline:{criterion:'A2'},goldenReference:{reference_id:'A1_AP-A1',normative:false},loadArtifact};

const resolved=await resolveSirTaskContract({...base,taskType:'CONTROL_BOUNDARY'});
if(resolved.taskType!=='CONTROL_BOUNDARY'||resolved.contractVersion!=='2.0.0') throw new Error('Resolver did not construct CONTROL_BOUNDARY SIR v2 contract.');
const lockedCapabilityFindings=resolved.lockedInputs.capability_findings as Array<{handle?:string}>;
if(lockedCapabilityFindings[0]?.handle!=='finding_001') throw new Error('Control contract did not receive verified materialized findings.');
if('source_context_packet' in resolved.lockedInputs||'source_mappings' in resolved.lockedInputs) throw new Error('Control contract leaked upstream source packet/mappings into its prompt boundary.');
...
const originalFinding=artifacts.get('FINDING_ARCHITECTURE')!;
const rawFinding={capabilityFindings:[],antipatternFindings:[],findingLogicNotes:[]};
artifacts.set('FINDING_ARCHITECTURE',{...originalFinding,output:rawFinding,outputHash:canonicalArtifactHash(rawFinding)});
await expectReject(()=>resolveSirTaskContract({...base,taskType:'CONTROL_BOUNDARY'}),'capability group must be a non-empty array');
artifacts.set('FINDING_ARCHITECTURE',originalFinding);

const badHandle=structuredClone(findings);
badHandle.capability[0]!.handle='finding_999';
artifacts.set('FINDING_ARCHITECTURE',{...originalFinding,output:badHandle,outputHash:canonicalArtifactHash(badHandle)});
await expectReject(()=>resolveSirTaskContract({...base,taskType:'CONTROL_BOUNDARY'}),'handle must be finding_001');
artifacts.set('FINDING_ARCHITECTURE',originalFinding);

const staleHash=structuredClone(findings);
staleHash.capability[0]!.title='Tampered finding after persistence.';
artifacts.set('FINDING_ARCHITECTURE',{...originalFinding,output:staleHash});
await expectReject(()=>resolveSirTaskContract({...base,taskType:'CONTROL_BOUNDARY'}),'output hash mismatch');
artifacts.set('FINDING_ARCHITECTURE',originalFinding);

artifacts.set('FINDING_ARCHITECTURE',{...originalFinding,taskContract:{...originalFinding.taskContract,contractVersion:'1.0.0'}});
await expectReject(()=>resolveSirTaskContract({...base,taskType:'CONTROL_BOUNDARY'}),'SIR v2 requires 2.0.0');
artifacts.set('FINDING_ARCHITECTURE',originalFinding);
...
await expectReject(()=>resolveSirTaskContract({...base,taskType:'CONTROL_BOUNDARY'}),'exact locator drifted');
artifacts.set('SOURCE_MAPPING',originalSource);

console.log(JSON.stringify({
  controlResolver:'PASS',
  transitiveSourceMappingIntegrity:'PASS',
  verifiedMaterializedFindingDependency:'PASS',
  sourceContextLeakageToControlPrompt:'PROHIBITED',
  rawFindingArtifact:'REJECTED',
  tamperedFindingHandle:'REJECTED',
  staleFindingOutputHash:'REJECTED',
  legacyFindingDependency:'REJECTED',
